...
The web UI is multilingual. English is the source of truth; each language is a
JSON file of the same keys under `core/assets/locales/`. Adding or completing a
language is a JSON-only change verified by a coverage test:

```bash
python -m pytest tests/unit/test_i18n_locales.py -q
```

Full guide (namespaces, `data-i18n` attributes, `{vars}`, contribution steps):
`docs/i18n.md`.
